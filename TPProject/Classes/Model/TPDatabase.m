...
- (void) dumpDatabase {
	...
	sql = [NSString stringWithFormat:@"select district_id, user_id, target_id, share, school_id, subject_id, grade, \
           elapsed, type, rubric_id, name, userdata_id, state, created, modified, description from userdata order by user_id, created"];
	returncode = sqlite3_prepare_v2(database, [sql UTF8String], -1, &statement, NULL);
	if (returncode == SQLITE_OK) {
		NSLog(@" Userdata:");
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			NSLog(@"  user=%d created=%s ID=%s target=%d district=%d share=%d school=%d subject=%d grade=%d elapsed=%d type=%d rubric=%d name=%s state=%d modified=%s, description=%s",
				  user_id, created, userdata_id, target_id, district_id, share, school_id, subject_id, grade, elapsed, type, rubric_id, name, state, modified, description);
		}
	} else {
...
- (void) dumpDatabaseShort {
	...
	if (returncode == SQLITE_OK) {
		...
		while (sqlite3_step(statement) == SQLITE_ROW) {
            ...
            int state         = (int)sqlite3_column_int(statement, 12);
            char *created     = (char *)sqlite3_column_text(statement, 13);
			NSLog(@"  ID=%s district=%d user=%d target=%d created=%s share=%d type=%d name=%s state=%d",
				  userdata_id, district_id, user_id, target_id, created, share, type, name, state);
		}
	} else {
...
- (void) getUserDataList:(NSMutableArray *)userdata_list target:(int)target_id filterUserId:(int)filterUserId {
    ...
    sql = [NSString stringWithFormat:@"\
           select district_id, user_id, target_id, share, school_id, subject_id, grade, elapsed, \
                  type, rubric_id, name, userdata_id, state, created, modified, description \
            from userdata \
           where target_id = %d %@ \
...
- (void) updateUserData:(TPUserData *)userdata setModified:(BOOL)setModified {
	...
	sql = [NSString stringWithFormat:@"insert into userdata \
           (district_id, user_id, target_id, share, school_id, subject_id, grade, elapsed, type, rubric_id, name, userdata_id, state, created, modified, description) \
...
- (NSString *) getUserDataXMLEncoding:(NSString *)userdata_id {
	...
	if (returncode == SQLITE_OK) {
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			char *modified = (char *)sqlite3_column_text(statement, 14);
            char *description = (char *)sqlite3_column_text(statement, 15);
			[userdata appendFormat:@"\n<userdata district=\"%d\" user=\"%d\" target=\"%d\" type=\"%d\" rubric=\"%d\" share=\"%d\" school=\"%d\" subject=\"%d\" grade=\"%d\" elapsed=\"%d\" state=\"%d\">",
             district_id, user_id, target_id, type, rubric_id, share, school_id, subject_id, grade, elapsed, state];
